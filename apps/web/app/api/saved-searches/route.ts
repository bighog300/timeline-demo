...
const validateRequest = (value: unknown): { ok: true; payload: CreateSavedSearchRequest } | { ok: false; message: string } => {
  if (!isRecord(value)) {
  ...
  return { ok: true, payload: { ...(value as CreateSavedSearchRequest), source } };
};
