...
describe("GmailSelectClient", () => {
  ...
  it("summarizes selected search results via timeline summarize endpoint", async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce({
        ...
        json: async () => ({
          resultCount: 2,
          nextPageToken: null,
          messages: [baseSearchResult, { ...baseSearchResult, id: "msg-2" }],
        }),
      })
      .mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => ({
          artifacts: [{ sourceId: "msg-1" }, { sourceId: "msg-2" }],
          failed: [],
        }),
      });
    ...
    fireEvent.change(screen.getByPlaceholderText(/Add sender email/i), {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: /Select all \(this page\)/i }),
    );
    fireEvent.click(
      screen.getByRole("button", { name: /Summarize selected now/i }),
    );
    ...
    expect(
      screen.getByRole("link", { name: /Open Timeline/i }),
  ...
  it("shows reconnect CTA when summarize returns reconnect_required", async () => {
    ...
    expect(
      await screen.findByText(/Google connection expired/i),
    ).toBeInTheDocument();
  ...
  it("updates summarize progress while running multiple batches", async () => {
    let resolveSecondBatch:
      ...
      | null = null;
    const secondBatchPromise = new Promise<{
      ok: boolean;
      status: number;
      json: () => Promise<{
        artifacts: Array<{ sourceId: string }>;
    ...
    const fetchMock = vi
      ...
      .mockResolvedValueOnce({
        ...
        status: 200,
        json: async () => ({
          artifacts: batchMessages
            .slice(0, 10)
            .map((message) => ({ sourceId: message.id })),
          failed: [],
        }),
      })
    ...
    expect(
      await screen.findByText(/Summarized 11 emails/i),
    ).toBeInTheDocument();
  });
  ...
  it("shows partial success summary when a later summarize batch fails", async () => {
    ...
    expect(
      await screen.findByText(
        /Summarized 10 of 15 emails\. 5 failed\. \(requestId: req-partial-500\)/i,
      ),
    ).toBeInTheDocument();
  });
  ...
  it("summarizes saved search with paging and shows progress states", async () => {
    ...
    expect(
      await screen.findByText(/Up to 5 pages \/ 50 emails\./i),
    ).toBeInTheDocument();
    fireEvent.click(
      screen.getByRole("button", { name: /Summarize up to 50 emails/i }),
    );

    await waitFor(() => {
      const gmailSearchCalls = fetchMock.mock.calls.filter(
        ([url]) => url === "/api/google/gmail/search",
      );
  ...
  it("enforces 50 email cap when summarizing saved search", async () => {
    ...
    const fetchMock = vi
      ...
      .mockResolvedValueOnce({
        ...
        status: 200,
        json: async () => ({
          artifacts: messagesPage.slice(0, 10).map((m) => ({ sourceId: m.id })),
          failed: [],
        }),
      })
      .mockResolvedValueOnce({
    ...
    expect(
      await screen.findByText(
        /Reached cap of 50 emails\. Refine your saved search or run again\./i,
      ),
    ).toBeInTheDocument();
    expect(
      await screen.findByText(/Summarized 50 emails/i),
    ).toBeInTheDocument();

    const summarizeCalls = fetchMock.mock.calls.filter(
      ([url]) => url === "/api/timeline/summarize",
    );
  ...
  it("routes to timeline with selectionSetId from selection bar summarize", async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce({
        ok: true,
      ...
      .mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => ({ set: { driveFileId: "sel-999" } }),
      })
      .mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => ({ artifacts: [{ sourceId: "msg-1" }], failed: [] }),
      });
    ...
    await waitFor(() => {
      expect(routerPush).toHaveBeenCalledWith(
        "/timeline?from=select&selectionSetId=sel-999",
      );
    });
  });
});
