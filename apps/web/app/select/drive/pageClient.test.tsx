...
describe("DriveSelectClient", () => {
      ...
      .mockResolvedValueOnce({
        ...
        json: async () => ({
          set: {
            ...
            title: "PDFs",
            kind: "drive_selection_set",
            query: { q: "trashed=false and mimeType='application/pdf'" },
          },
        }),
      })
      .mockResolvedValueOnce({
  ...
  it("summarizes selected drive results", async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce({
        ...
        status: 200,
        json: async () => ({
          artifacts: [{ sourceId: "file-1" }, { sourceId: "file-2" }],
          failed: [],
        }),
      });
  ...
  it("caps summarize saved search collection at 50 files", async () => {
    ...
    const fetchMock = vi
      ...
      .mockResolvedValueOnce({
        ...
        status: 200,
        json: async () => ({
          set: { kind: "drive_selection_set", query: { q: "trashed=false" } },
        }),
      })
      .mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => ({ files: firstPage, nextPageToken: "next-2" }),
      })
      .mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => ({ files: secondPage, nextPageToken: "next-3" }),
      })
      .mockResolvedValue({
        ok: true,
        status: 200,
    ...
    const summarizeCalls = fetchMock.mock.calls.filter(
      (call) => call[0] === "/api/timeline/summarize",
    );
  ...
  it("copies file id and shows notice", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: /Copy file ID for Doc/i }),
    );
  ...
  it("disables app folder toggle when drive folder is missing", async () => {
    ...
    const toggle = await screen.findByRole("checkbox", {
      name: /Limit to app folder/i,
    });
    expect(toggle).toBeDisabled();
    expect(
      screen.getByText("Provision Drive folder on /connect."),
    ).toBeInTheDocument();
  });
  ...
  it("routes to timeline with selectionSetId from selection bar summarize", async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => ({ set: { driveFileId: "sel-123" } }),
      })
      .mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => ({ artifacts: [{ sourceId: "file-1" }], failed: [] }),
      });
    ...
    await waitFor(() => {
      expect(routerPush).toHaveBeenCalledWith(
        "/timeline?from=select&selectionSetId=sel-123",
      );
    });
  });
});
